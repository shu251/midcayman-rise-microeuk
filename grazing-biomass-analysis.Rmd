...
```{r Separate comma delim FLP counts}
counts_occur <- counts %>%
...
```{r import and set up prokaryote DAPI count data}
...
Can include years 2012 and 2013, seeking clarification on these data.
## Prokaryote biomass-literature
...
```{r Apply function to obtain slope}
...
```{r Plot results from bag experiments, fig.height=6, fig.width=7}
...
```{r plot igt only, fig.height=4, fig.width=5}
igt_regression_noTf %>% 
  # filter(SizeFrac == "total_euks") %>% 
  # filter(TYPE != "IGT") %>% 
  unite(EXPERIMENT, SITE, Experiment, sep = " ", remove = FALSE) %>% 
...
```{r Plot average FLP in controls, fig.height=6, fig.width=7}
dtaf_avg %>% 
  filter(Site != "IGT") %>% 
...
```{r Plot for IGT, fig.height=3, fig.width=4}
dtaf_avg %>% 
  filter(Site == "IGT") %>% 
  ggplot(aes(x = TimePoint, y = Avg_cellsperml, fill = Name, shape = Site)) +
  geom_rect(data = filter(dtaf_avg, TimePoint == "T0", Site == "IGT"), aes(
...
```{r Plot grazing rate by site, fig.height=4, fig.width=6}
# svg("", h =, w = )
...
Explaination of units for table with calculated values.
...
```{r Import and process biovolumes}
...
```{r Compare carbon ranges, fig.height=7, fig.width=10}
# Hu-measured
...
```{r Plot grazing rates within MCR, fig.height=7, fig.width=6}
...
```{r Plot grazing rate, fig.height=5, fig.width=6}
# scale_y_continuous(labels = function(x) format(x, scientific = TRUE)) +
...
Plot grazing rates
```{r Plot grazing rates from both sites}
allrates <- all_vents %>% 
...
```{r Plot results from lm, fig.height=7, fig.width=15}
regression_results %>% 
