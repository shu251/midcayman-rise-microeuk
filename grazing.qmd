...
#               select(FIELD, NAME, EXP, SAMPLE, TimePoints, starts_with("EXP_"), starts_with("CTRL_"), PROK_ml, PROK_sem, EUK_ml, EUK_sem, RATE_min, GRAZING_EFFECT_hr, CLEARANCE_RATE_ml, BAC_TURNOVER_PERC)), file = "output-data/table1-grazing-exp-list.txt", delim = "\t")
```
...
hist(counts_cellsml_axial$eukCONC)
```
...
# table_wcalcs_biomass_bounded$Euk
```
...
## Comparisons for results section

Function to output mean, min and max for these observations
* Non-vent vs. vent
* Piccard vs. VD (vent only)
* shipboard vs. in situ (vent only)

```{r}
head(table_wcalcs_biomass_bounded)

stats_from_mcr <- function(df, group, variable_input) {
    VAR <- enquo(variable_input)
    GROUP <- enquo(group)
  df %>% 
    select(!!GROUP, !!VAR) %>% distinct() %>% 
    group_by(!!GROUP) %>% 
...
### Prokaryote cells per ml
```{r}
# Non-vent vs. vent
stats_from_mcr(table_wcalcs_biomass_bounded, SAMPLE_TYPE_BIN, PROK_ml)
# Piccard vs. VD (vent only)
stats_from_mcr((table_wcalcs_biomass_bounded %>% 
                  filter(SAMPLE_TYPE_BIN == "Vent")), 
               FIELD, PROK_ml)
# shipboard vs. in situ (vent only)
stats_from_mcr((table_wcalcs_biomass_bounded %>% 
...
### Eukaryote cells per ml
```{r}
# Non-vent vs. vent
stats_from_mcr(table_wcalcs_biomass_bounded, SAMPLE_TYPE_BIN, EUK_ml)
# Piccard vs. VD (vent only)
stats_from_mcr((table_wcalcs_biomass_bounded %>% 
                  filter(SAMPLE_TYPE_BIN == "Vent")), 
               FIELD, EUK_ml)
...
### Grazing rate
```{r}
names(table_wcalcs_biomass_bounded)
# Non-vent vs. vent
stats_from_mcr(table_wcalcs_biomass_bounded, SAMPLE_TYPE_BIN, GRAZING_EFFECT_hr)
# Piccard vs. VD (vent only)
stats_from_mcr((table_wcalcs_biomass_bounded %>% 
                  filter(SAMPLE_TYPE_BIN == "Vent")), 
               FIELD, GRAZING_EFFECT_hr)
# shipboard vs. in situ (vent only)
stats_from_mcr((table_wcalcs_biomass_bounded %>% 
                  filter(SAMPLE_TYPE_BIN == "Vent")), 
               EXP, GRAZING_EFFECT_hr)
```
Grazing rate without zeros
```{r}
names(table_wcalcs_biomass_bounded)
# Non-vent vs. vent
stats_from_mcr((table_wcalcs_biomass_bounded %>% 
                  filter(GRAZING_EFFECT_hr > 0)), SAMPLE_TYPE_BIN, GRAZING_EFFECT_hr)
...
table_wcalcs_biomass_bounded %>% 
  select(-TimePoints, -EXP_REPS, -EXP_VOL, -CTRL_REPS, -CTRL_VOL, -type, -type_site,
         -(ends_with("_sem")), -(ends_with("_min")), -(ends_with("_max"))) %>% 
  pivot_longer(cols = c(ends_with("BIOMASS"), ends_with("_ml"), ends_with("_hr"), ends_with("_day"))) %>% 
  group_by(EXP, SAMPLE_TYPE_BIN, FIELD, name) %>%
...
View(table_wcalcs_biomass_bounded %>% 
  select(-TimePoints, -EXP_REPS, -EXP_VOL, -CTRL_REPS, -CTRL_VOL, -type, -type_site,
         -(ends_with("_sem")), -(ends_with("_min")), -(ends_with("_max"))) %>% 
  pivot_longer(cols = c(ends_with("BIOMASS"), ends_with("_ml"), ends_with("_hr"), ends_with("_day"))) %>% 
  group_by(EXP, SAMPLE_TYPE_BIN, name) %>%
    summarize(MEAN_VALUE = mean(value)) %>% 
...
# euk_prok_flp_rate <- 
  ...
  (grazing_rate + scale_y_log10(limits = c(1,1e4))) +
  patchwork::plot_layout(ncol = 1, guides = "collect") + guide_area() | patchwork::plot_annotation(tag_levels = "a")
```
...
plot_metadata <- env_params %>% 
  ...
  select(SITE, VENT, SAMPLETYPE, TEMP, DEPTH, EXP, starts_with("PROK_"), starts_with("EUK_"), starts_with("GRAZING_EFFECT_hr"), pH, Mg, H2., CH4, H2S)
...
euk_temp <- plot_metadata %>% 
  ...
  geom_point(aes(shape = EXP), color = "black", size = 4) +
  scale_y_log10() + 
  scale_x_log10() +
  scale_shape_manual(values = c(21, 24)) +
...
allrates <- all_vents %>% 
  ...
  distinct() %>% 
    ggplot(aes(y = GRAZING_EFFECT_hr, x = NAME, fill = FIELD, shape = EXP)) +
    geom_errorbar(aes(ymax = (GRAZING_EFFECT_hr_max), ymin = (GRAZING_EFFECT_hr_min)), 
                  width = 0.2, position = position_dodge(width = 0.4)) +
    geom_point(stat = "identity", aes(fill = FIELD, shape = EXP),
               color = "black", size = 3, position = position_dodge(width = 0.4)) +
...
TO DO: find a tidy model tutorial to work with.
...
```{r, fig.width=, fig.height=8, fig.width=15}
...
```{r, fig.height=8, fig.width=15}
est_predicted(all_vents %>%  filter(GRAZING_EFFECT_hr > 0), "All samples where grazing was detected")
```
