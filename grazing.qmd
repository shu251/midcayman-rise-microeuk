...
counts_cellsml_avg %>% 
  ...
  ## Plot with circle and diamond:
  # ggplot(aes(x = NameOrder, y = MEAN, group = NameOrder,
  #                                         fill = VARIABLE,
  #                                         shape = EXP_CATEGORY)) +
  # geom_jitter(size = 3, stroke = 1, color = "black", aes(fill = VARIABLE, 
  #                                       shape = EXP_CATEGORY)) +
  # geom_boxplot(alpha = 0.1) +
...
table_grazerate_wflp_wprok_weuk <- table_grazerate_wflp %>% 
  ...
  left_join(euk_prok_counts, join_by(Experiment_rep == Experiment_rep, Name == Name, FIELD == Site)) %>% 
...
```{r}
# April 13, 2023
write_delim(table_wcalcs, file = "output-data/table1-grazing-exp-list.txt", delim = "\t")
```
...
# For vent-only samples, compare IGT vs shipboard experiment reulst for biovolume
biov_calc %>% 
  filter(VENT_BSW != "BSW") %>% 
  group_by(EXP) %>% 
...
by_biovol_plot <- pgC_ml_biov_bysample %>% 
  mutate(NameOrder = factor(Name, levels = vent_ids, labels = vent_fullname)) %>%
  mutate(SiteOrder = factor(Site, levels = site_ids, labels = site_fullname)) %>% 
  ggplot(aes(x = Experiment_rep, y = (pgC_ml_mean/1000))) +
...
by_sizefrac_plot <- pgC_ml_sizefrac_bysample %>% 
   mutate(NameOrder = factor(Name, levels = vent_ids, labels = vent_fullname)) %>%
  mutate(SiteOrder = factor(Site, levels = site_ids, labels = site_fullname)) %>% 
  ggplot(aes(x = Experiment_rep, y = (pg_C_ml/1000))) +
...
est_prok_carbon <- table_wcalcs %>% 
  select(FIELD, NameOrder, SiteOrder, Experiment_rep, starts_with("PROK_"), RATE_minute, CLEARANCE_RATE_ml, GRAZING_EFFECT_hr) %>% 
...
carbon_consumed <- est_prok_carbon %>% 
  ...
  select(FIELD, NameOrder, SiteOrder, Experiment_rep, PROK_ml, GRAZING_EFFECT_hr, CLEARANCE_RATE_ml, ugC_cell, ugC_ml = value) %>% 
...
Report grazing rates from shipboard and IGT experiments. Differentiate vent and non-vent samples.
```{r}
table_wcalcs %>% 
    mutate(SAMPLE_TYPE_BIN = case_when(
    NameOrder == "Background" ~ "Non-vent",
...
```
```{r}
grazing_rate + scale_y_log10(limits = c(1,1e5))
```
...
euk_temp <- grazing_wenv %>% 
  ...
  ggplot(aes(x = GRAZING_EFFECT_hr, y = EUK_ml, 
             fill = TEMP, color = TEMP, shape = EXP_TYPE)) +
